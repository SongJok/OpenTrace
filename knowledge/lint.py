...
async def run_knowledge_lint(
    ...
    *,
    ...
    scoped_resource_ids: set[str] | None = None
    if source_ids is not None:
        if not source_ids:
            return {"findings": [], "open_count": 0}
        scoped_versions = set(
            (
                await db.execute(
                    select(KnowledgeSource.active_version_id).where(
        ...
        page_conditions.append(KnowledgePage.source_version_id.in_(scoped_versions))
        claim_conditions.append(KnowledgeClaim.source_version_id.in_(scoped_versions))
        relation_conditions.append(KnowledgeRelation.source_version_id.in_(scoped_versions))
        source_conditions.append(KnowledgeSource.id.in_(source_ids))
    if owner_id:
    ...
    claims = list(
        (await db.execute(select(KnowledgeClaim).where(*claim_conditions))).scalars().all()
    )
    relations = list(
        (await db.execute(select(KnowledgeRelation).where(*relation_conditions))).scalars().all()
    )
    sources = list(
        (await db.execute(select(KnowledgeSource).where(*source_conditions))).scalars().all()
    )
    if source_ids is not None:
        scoped_resource_ids = (
    ...
    active_version_ids = {
        source.active_version_id for source in sources if source.active_version_id
    }
    for claim in claims:
        if (
    ...
    for page in pages:
        if (
            ...
            and page.page_type != "overview"
            and page.id not in claims_by_page
        ):
            findings.append(
                {
        ...
        if (
            page.status == "published"
            and page.page_type != "overview"
            and page.id not in relation_targets
        ):
            findings.append(
                {
                    "code": "orphan_page",
    ...
    for source in sources:
        ...
        if source.status == "published" and source.updated_at:
            ...
            cutoff = datetime.now(UTC) - timedelta(days=max(1, settings.knowledge_stale_after_days))
    ...
    existing_issues = list(
        (
            await db.execute(
                select(KnowledgeLintIssue).where(
                    *issue_conditions, KnowledgeLintIssue.status == "open"
                )
            )
        )
        .scalars()
