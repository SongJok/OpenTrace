...
Beta 覆盖 `/api/v2/responses` 的持久化创建、Worker 租约执行、工具与审批、SSE 续传、企业准入、实际 token/成本结算，以及企业经营报告灯塔。旧 `/api/v1/chat`、`/api/v1/tasks` 和 Cognitive Runtime 不在 Beta 主路径。
...
2. 观察完成率、P95 时延、租约恢复、审批积压、工具失败、报告证据完整率和成本偏差。
