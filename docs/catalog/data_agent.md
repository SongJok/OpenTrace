...
- **大规模 Schema 目录**：元数据同步独立于业务 SQL 的 500 行结果保护器，按批次读取并通过可搜索分页目录展示
...
.sql/.txt 批量上传（每批最多 100 个文件）
  ...
  → 合法查询草案可作为低可信业务参考，人工审核发布后成为稳定治理资产
  → 检索“同一 data_source_id + 当前 Schema 可证明合法”的查询资产
...
都只生成草案，不接受客户端通过 `dry_run=false` 绕过确认。数据库页面通过草案执行 API
...
会自动进入隔离区。检索库中尚未发布、但已通过上传时 Schema/敏感字段/只读 AST 校验的
`SELECT/WITH` 可以作为 DataAgent V2 和统一草案服务的低可信参考，用于识别业务词、表、字段、
指标、JOIN 和历史问法；它们不能直接执行，也不能绕过最终候选的当前 Schema、只读和确认校验。
只有已审核发布且已验证的资产才能成为稳定治理口径和可复用执行资产。

### 2.2 Schema 业务标注与 SQL 知识接入

物理数据库没有表或字段注释时，可以在数据库页面的“Schema 语义”页签手工维护表名、
...
会创建为待审核候选。未发布查询可以帮助无注释数据库完成实体和指标理解，但其来源会标记为
`sql_asset_reference`；只有确认后的知识才作为稳定治理事实。
...
已验证 JOIN、已发布 SQL 资产和通过当前 Schema 范围校验的历史查询参考，再交给模型生成 1–N 个
候选。DataAgent V2 使用相同范围把资产中的表、字段、指标和 JOIN 注入认知上下文；明确表名、
字段、`id=值` 或表结构意图时，不再因为数据库缺少 COMMENT 而泛化追问。复合问题会把每个选中指标保存为
...
1. 在数据库页面“SQL 资产”页选择业务域、负责人和语料分区，再多选 `.sql/.txt` 文件上传；
