...
规划后的“已有信息意图消解”会再次核对父链指代、个人记忆、企业认知、企业大脑、公司 Skill、
附件和个人业务状态。已有信息足以补全或只读 RAG 能自行研究时，不向用户重复索取资料，并清理
计划中已经失效的澄清步骤。关键目标/范围缺失、来源冲突、权限问题、写入风险和 SQL 候选选择
仍保留为最小必要澄清；决策原因写入 `context_manifest.intent_resolution` 供事件与最终元数据审计。

Manager 同时维护工具调用和语义结果指纹。连续两轮只有重复调用、重复结果、失败或空结果
时，会发出 `opentrace.loop.no_progress` 并进入禁用工具的最终合成：已有可靠证据可以继续
形成回答，外部数据不可用则明确说明能力缺口，不再把无效循环误报成任务过于复杂。通用
工具函数包装层中的失败状态也会被提升为真实失败，Web Intelligence 不会把搜索服务错误
字符串当作网页证据。
...
强化和时间衰减共同参与排序。关系图不能扩大用户、租户、工作区或 conversation
