...
RRF 将每个 `lane + query_variant` 视为独立排名列表，并按稳定的来源实体 ID 合并重复命中。
同一证据被多个查询变体稳定召回时保留完整分数，只命中少数变体时最多做有限下调。通道权重
只应用一次，排名信号不能把低于绝对相关性门槛的片段抬升为可回答证据。证据协议同时保留
`raw_score`、`weighted_score`、`rrf_score`、`retrieval_hit_count`、命中查询和各列表排名，便于
离线复现和分数校准。
...
`evals/datasets/rag.jsonl` 同时覆盖文档引用、租户/知识空间隔离、证据不足拒答、冲突拒答、
低相关性门禁和多查询稳定性。`evals.runner` 对真实主链产出的证据 ID 计算以下确定性指标：

| 指标 | 作用 |
|------|------|
| `precision_at_k` | Top-K 中相关证据的占比 |
| `recall_at_k` | 标注相关证据在 Top-K 中的召回比例 |
| `mrr_at_k` | 第一条相关证据的排名质量 |
| `ndcg_at_k` | 多条相关证据的整体排序质量 |
| `hit_rate_at_k` | Top-K 是否至少命中一条相关证据 |
| `irrelevant_at_k` | Top-K 中无关证据数量 |

生产发布必须使用 `scripts/run_enterprise_evals.py --require-results --results-dir ...` 读取绑定
Response ID 和采集时间的真实主路径输出；`--validate-contracts` 只校验数据集结构，不能冒充
产品质量。`--minimum-metric rag.recall_at_k=...` 等参数
可把数据集指标均值设为硬门槛；报告保留每个案例的指标、失败断言和数据集均值。
...
| `max_score` | 最高匹配分数 |
| `confidence` | 综合置信度 |
| `opentrace_rag_retrieval_total` | 按可回答状态、查询类型和企业 grounding 统计检索结果 |
| `opentrace_rag_retrieval_duration_seconds` | RAG 检索、融合和证据处理耗时 |
| `opentrace_rag_evidence_count` | 最终去重证据数量分布 |
| `opentrace_rag_max_score` | 校准后最高相关性分数分布 |
| `opentrace_rag_anchor_score` | 查询与证据锚定分数分布 |
...
2. 并行执行企业知识、文档、LLMWiki 和记忆检索
3. 默认限制最多 3 个查询变体，最高 6 个，避免延迟爆炸
4. 每个通道调用由 `RAG_LANE_TIMEOUT_SECONDS` 设置独立截止时间，一路超时不取消其他通道
5. 主文档通道全部失败时不重复调用同一服务做 fallback，防止故障期间延迟翻倍

**阶段 5：结果处理**
1. 多查询融合：按稳定来源实体 ID 合并同一证据，并记录每个查询变体的命中排名
2. 校准 RRF：通道权重只应用一次，排名信号只调整相对顺序，不抬高绝对相关性
3. 神经重排序：使用 qwen3-vl-rerank 模型优化排序
4. 质量评估：计算置信度、答案可回答性和证据质量
...
4. **可观测性**：输出详细的质量评估指标
5. **异步优先**：所有 IO 操作异步执行
6. **不制造假阴性**：区分「可用但无命中」「部分降级」和「全部不可用」，故障时不宣称知识库无内容
7. **低敏失败轨迹**：`rag_trace` 仅记录通道、阶段、变体序号、失败类型和耗时，不持久化异常正文或查询正文
