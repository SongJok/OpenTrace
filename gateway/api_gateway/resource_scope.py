...
def accessible_data_sources_statement(
    *,
    ...
    tenant_id, workspace_id = normalized_tenant_scope(tenant_metadata)
    allowed = [name for name, rank in PERMISSION_RANK.items() if rank >= PERMISSION_RANK.get(required_permission, 99)]
    permission_exists = exists(select(ResourcePermission.id).where(
        ...
        or_(ResourcePermission.expires_at.is_(None), ResourcePermission.expires_at > datetime.now(UTC)),
    ))
    stmt = select(DataSource).where(
        DataSource.tenant_id == tenant_id,
        DataSource.workspace_id == workspace_id,
...
async def get_accessible_data_source(
    ...
    *,
    ...
    result = await db.execute(accessible_data_sources_statement(
        user_id=user_id,
        tenant_metadata=tenant_metadata,
        data_source_id=data_source_id,
