...
async def register(req: RegisterRequest, db: AsyncSession = Depends(get_db)) -> LoginResponse:
...
async def login(
    ...
    if not user.is_active:
        raise AppException(ErrorCodes.PERMISSION_DENIED.code, message="Account disabled")
    token = _create_token(user.id, user.email)
