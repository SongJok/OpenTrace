...
async def _validate_attachments(
    ...
    *,
    ...
    if set(rows) != set(attachment_ids):
        raise AppException(
            ErrorCodes.RESOURCE_NOT_FOUND.code, message="附件不存在或不属于当前 Conversation"
        )
    ...
    if extension.assistant_profile_id:
        owned = await db.scalar(
            select(AssistantProfile.id).where(
                AssistantProfile.id == extension.assistant_profile_id,
...
async def _resolve_parent(
    ...
    *,
    ...
    parent_id = (
        request.previous_response_id or request.parent_response_id or session.active_response_id
    )
    if not parent_id:
        return None
    parent = await _load_response(db, str(parent_id), user, tenant_id, session.workspace_id)
    if parent.conversation_id != session.id:
        raise AppException(
            ErrorCodes.PARAM_INVALID.code, message="previous_response_id 不属于当前 Conversation"
        )
    return parent.id
                ...
                (
                    await db.execute(
                        ...
                        .where(
                            ...
                            ResponseEvent.sequence_number > cursor,
                        )
...
async def _wait_for_response(
    response_id: str, *, timeout_seconds: float = 120.0
) -> tuple[ResponseRecord, list[ResponseItem]]:
    ...
    while True:
        async with AsyncSessionLocal() as db:
            ...
            if (
                ...
                or record.status == "requires_action"
...
async def create_response(
    ...
    chat_constitution = await load_effective_chat_constitution(
        db,
        tenant_id=tenant_id,
        workspace_id=workspace_id,
    )
    constitution_decision = evaluate_chat_constitution(
        query,
        constitution=chat_constitution,
    )
    if constitution_decision.decision == "block":
        ...
        add_chat_constitution_audit(
            ...
            tenant_id=tenant_id,
            workspace_id=workspace_id,
            constitution_version=chat_constitution.version,
        ...
        await db.commit()
        raise AppException(
            ErrorCodes.CHAT_CONSTITUTION_BLOCKED.code,
            message=str(chat_constitution.rules["block_message"]),
            details={
                "reason_code": constitution_decision.reason_code,
                "categories": list(constitution_decision.categories),
                "constitution_version": chat_constitution.version,
                "request_id": request_id,
            },
        )
    if idempotency_key:
        ...
        if existing:
            if existing.user_id != current_user.id:
                raise AppException(
                    ErrorCodes.PERMISSION_DENIED.code, message="幂等键不可跨用户复用"
                )
            if request.stream:
            ...
            items = (
                (
                    await db.execute(
                        select(ResponseItem)
                        .where(ResponseItem.response_id == existing.id)
                        .order_by(ResponseItem.sequence_number)
                    )
                )
                .scalars()
    ...
    await append_event(
        ...
        payload={"response_id": response_id, "status": "queued"},
    )
...
async def get_response(
    ...
    if stream:
        return StreamingResponse(
            _event_stream(record.id, starting_after=starting_after),
            media_type="text/event-stream",
            headers={"Cache-Control": "no-cache", "X-Accel-Buffering": "no"},
        )
    items = (
    ...
    events = (
        (
            await db.execute(
                select(ResponseEvent)
                .where(
                    ResponseEvent.response_id == response_id, ResponseEvent.sequence_number > cursor
                )
                .order_by(ResponseEvent.sequence_number)
            )
        )
    ...
    rows = (
        (
            await db.execute(
                ...
                .where(
                    ResponseRecord.conversation_id == source.conversation_id,
                    ResponseRecord.parent_response_id == source.parent_response_id,
        ...
        "items": [
            {
                ...
                "active": bool(session and session.active_response_id == row.id),
            }
            for row in rows
        ]
    }
...
async def retry_response(
    ...
    return await create_response(
        http_request, ResponseCreateRequest.model_validate(payload), None, current_user, db
    )
