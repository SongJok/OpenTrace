"""
DEPRECATED — OpenTrace Cognitive OS vNext uses RuntimeGateway + CognitiveExecutive only.

Do not import CognitiveOrchestratorV4 in new code. Kept for contract tests and migration reference.
"""
...
from kernel.cognitive_controls import passes_relevance_anchor
...
class CognitiveOrchestratorV4:
    ...
    def __init__(self, timeout_sec: int = 30, max_parallel: int = 5) -> None:
        if not bool(getattr(settings, "kernel_orchestrator_v4_enabled", False)):
            raise RuntimeError(
                "CognitiveOrchestratorV4 is disabled. Use kernel.runtime_gateway.RuntimeGateway "
                "(Cognitive Runtime V2) as the sole orchestration path."
            )
        ...
        # Phase 2: UnifiedOrchestrator components (lazy)
        self._unified_orchestrator: Any = None
        self._execution_runtime: Any = None
        self._evidence_bus: Any = None
        # Cognitive Runtime components (lazy)
        self._cognitive_planner: Any = None
        self._rewrite_engine: Any = None
        self._understanding_engine: Any = None

    # ── Phase 2: UnifiedOrchestrator pipeline ──────────────────────────────

    def _ensure_unified(self) -> tuple[Any, Any, Any]:
        """Lazily initialize UnifiedOrchestrator + ExecutionRuntime + EvidenceBus."""
        if self._unified_orchestrator is None:
            from kernel.runtime.capability import capability_registry
            from kernel.runtime.orchestrator import UnifiedOrchestrator
            from kernel.runtime.executor import ExecutionRuntime
            from kernel.runtime.evidence_bus import EvidenceBus

            self._unified_orchestrator = UnifiedOrchestrator(
                capability_registry=capability_registry,
            )
            self._execution_runtime = ExecutionRuntime(
                capability_registry=capability_registry,
                timeout_sec=getattr(self.dispatcher, "timeout_sec", 30),
                max_parallel=self.max_parallel,
            )
            self._evidence_bus = EvidenceBus()
        return self._unified_orchestrator, self._execution_runtime, self._evidence_bus

    def _ensure_cognitive_runtime(self) -> tuple[Any, Any, Any]:
        """Lazily initialize CognitivePlanner + RewriteEngine + UnderstandingEngine."""
        ...
        return self._cognitive_planner, self._rewrite_engine, self._understanding_engine
    ...
    async def _process_unified(
        ...
        """Clean pipeline: one-shot plan → execute → collect evidence → fuse → critique."""
        from kernel.runtime.context import RuntimeContext

        # ── CognitiveExecutive shortcut: all-in-one pipeline ──
        if settings.kernel_runtime_cognitive_planner_enabled and settings.kernel_runtime_capability_graph_enabled:
            try:
                from kernel.runtime.cognitive_executive import CognitiveExecutive
                ce = CognitiveExecutive()
                # Build context
                ctx = RuntimeContext(
                    request_id=str(req.metadata.get("request_id", "")),
                    session_id=req.session_id,
                    user_id=req.user_id,
                    query=masked_query,
                    conversation_history=req.history or [],
                    conversation_state=req.conversation_state,
                    memory_context=str(req.metadata.get("memory_context", "")),
                    preference_context_block=str(req.metadata.get("user_preference_context_block", "")),
                    data_source_context=data_source_context,
                    attachment_contexts=req.metadata.get("attachment_contexts") or [],
                    force_mode=force_mode,
                    web_enabled=bool(req.metadata.get("web_enabled", False)),
                    enabled_skills=req.metadata.get("enabled_skills") or [],
                    adaptive_profile=adaptive_profile,
                )
                result = await ce.execute(masked_query, ctx, event_cb=event_cb)
                # Restore PII
                final_answer = result.answer
                for placeholder, original in pii_mapping.items():
                    final_answer = final_answer.replace(placeholder, original)
                elapsed_ms = int((monotonic() - t0) * 1000)
                return OrchestratorV4Response(
                    content=final_answer,
                    route="cognitive_executive",
                    strategy=getattr(result.plan, "merge_strategy", "union") or "union",
                    passed_validation=getattr(result.critic_result, "passed", True),
                    validation_score=getattr(result.fusion_result, "confidence", 0.0),
                    hallucination_risk=getattr(result.critic_result, "hallucination_risk", 0.0),
                    intent_category=ctx.force_mode or "general",
                    metadata={
                        "orchestrator_version": "cognitive_executive",
                        "force_mode": force_mode,
                        "adaptive_profile": adaptive_profile,
                        "subtask_count": len(getattr(result.plan, "subtasks", [])),
                        "agent_count": len(result.evidence_objects),
                        "fusion": {"method": getattr(result.fusion_result, "method", "")},
                        "critic": getattr(result.critic_result, "to_dict", lambda: {})() if result.critic_result else {},
                    },
                )
            except Exception as exc:
                logger.warning("CognitiveExecutive shortcut failed, falling back to step-by-step", error=str(exc))

        unified, executor, evidence_bus = self._ensure_unified()

        # 1. Build RuntimeContext from request metadata
        ...
        # 2. Cognitive Runtime pipeline (when enabled)
        canonical_query = masked_query
        understanding: Any = None
        use_cognitive_planner = settings.kernel_runtime_cognitive_planner_enabled

        if settings.kernel_runtime_rewrite_enabled:
            _, rewrite_engine, _ = self._ensure_cognitive_runtime()
            canonical = await rewrite_engine.rewrite(masked_query, ctx)
            canonical_query = canonical.canonical_query
            logger.debug("RewriteEngine produced canonical query", original=masked_query[:80], canonical=canonical_query[:80])

        if settings.kernel_runtime_understanding_enabled:
            _, _, understanding_engine = self._ensure_cognitive_runtime()
            from kernel.runtime.objects import RuntimeCanonicalQuery as RCQ
            if not settings.kernel_runtime_rewrite_enabled:
                # Build minimal canonical if rewrite wasn't run
                canonical_obj = RCQ(canonical_query=masked_query, original_query=masked_query)
            else:
                canonical_obj = RCQ(canonical_query=canonical_query, original_query=masked_query)
            understanding = await understanding_engine.understand(canonical_obj, ctx)
            logger.debug("UnderstandingEngine produced understanding", goal=understanding.explicit_goal[:80])

        # 3. One-shot plan — CognitivePlanner or UnifiedOrchestrator
        if use_cognitive_planner:
            cognitive_planner, _, _ = self._ensure_cognitive_runtime()
            plan = await cognitive_planner.plan(canonical_query, ctx, understanding=understanding)
        else:
            plan = await unified.plan(canonical_query, ctx)

        # 3. XAI trace
        if xai_tracer and xai_trace_id:
            try:
                xai_tracer.record_decision(
                    xai_trace_id, "PLAN", "plan_generated",
                    {"subtask_count": len(plan.subtasks), "merge_strategy": plan.merge_strategy},
                    f"Unified orchestrator generated {len(plan.subtasks)} subtasks",
                )
            except Exception:
                pass

        # 4. Execute
        agent_results = await executor.execute(plan, ctx=ctx, event_cb=event_cb)

        # 5. Publish evidence
        await evidence_bus.publish_results(agent_results)

        # 6. Fuse + Critique — V2 evidence pipeline (when enabled) or legacy V1 path
        evidence_list_raw = await evidence_bus.collect()

        if settings.kernel_runtime_evidence_fusion_critic_enabled:
            # ── V2 Evidence Pipeline: Evidence → FusionV2 → CriticV2 ──
            from kernel.runtime.fusion import FusionEngineV2
            from kernel.runtime.critic import CriticEngineV2

            fusion_v2 = FusionEngineV2()
            critic_v2 = CriticEngineV2()

            # Collect evidence objects directly from agent results
            ...
            if not evidence_objects:
                evidence_objects = evidence_list_raw

            fusion_result = await fusion_v2.fuse(
                query=masked_query,
                ctx=ctx,
                evidence_list=evidence_objects,
            )
            critic_result = await critic_v2.evaluate(
                query=masked_query,
            ...
            final_fused_context = fusion_result.merged_context
            final_confidence = fusion_result.confidence
            use_v2_fusion = True
        else:
            # ── V1 Legacy: Evidence → ToolResult → FusionInput → FusionEngine ──
            from kernel.fusion_engine.models import FusionInput, ToolResult
            ...
            for ev in evidence_list_raw:
                tool_results.append(ToolResult(
                    ...
                    data=ev.content,
                    confidence=ev.credibility_score,
                    result_refs=[ev.metadata],
                ))
            ...
            fusion_output = self.fusion_engine.run(fusion_input)

            # 7. Critique (validation only, no replan)
            from kernel.critic_engine.models import CriticInput
            critic_input = CriticInput(
                query=masked_query,
                answer=fusion_output.merged_context,
                fusion_context=fusion_output.merged_context,
                fusion_confidence=fusion_output.confidence,
                adaptive_profile=adaptive_profile,
            )
            critic_output = self.critic_engine.run(critic_input)
            final_fused_context = fusion_output.merged_context
            final_confidence = fusion_output.confidence
            use_v2_fusion = False

        # 8. Restore PII
        final_answer = final_fused_context or ""
        if not use_v2_fusion:
            final_answer = critic_output.improved_answer or final_fused_context or ""
        for placeholder, original in pii_mapping.items():
            final_answer = final_answer.replace(placeholder, original)

        # 9. Store artifact in workspace + memory (when enabled)
        ...
        if settings.kernel_runtime_artifact_composer_enabled:
            try:
                ...
                intent_category = getattr(plan, "intent_category", "") or ""
                if isinstance(intent_category, str) and not intent_category:
                    intent_category = ctx.force_mode or "general"

                stored_artifact = composer.compose(
                    query=masked_query,
                    fusion_result=fusion_result if use_v2_fusion else fusion_output,
                    critic_result=critic_result if use_v2_fusion else critic_output,
                    session_id=req.session_id,
                    turn_id=str(req.metadata.get("request_id", "")),
                    intent_category=intent_category,
                )

                if settings.kernel_runtime_workspace_enabled:
                    wm = WorkspaceManager()
                    ws = wm.get_or_create(req.session_id)
                    ws.add_artifact(stored_artifact)

                try:
                    from kernel.runtime.memory_fabric import memory_fabric
                    await memory_fabric.record_event(
                        event_type="artifact_created",
                        payload={
                            "artifact_id": stored_artifact.artifact_id,
                            "name": stored_artifact.name,
                            "type": stored_artifact.artifact_type,
                            "session_id": req.session_id,
                        },
                        session_id=req.session_id,
                    )
                ...
                logger.debug("Artifact storage skipped", error=str(exc))

        elapsed_ms = int((monotonic() - t0) * 1000)

        passed_validation = not getattr(critic_output, "need_fix", False) if not use_v2_fusion else getattr(critic_result, "passed", True)
        validation_score = final_confidence
        hallucination_risk = 0.0 if passed_validation else 0.25

        return OrchestratorV4Response(
            content=final_answer,
            route="unified",
            strategy=plan.merge_strategy or "union",
            passed_validation=passed_validation,
            validation_score=validation_score,
            hallucination_risk=hallucination_risk,
            intent_category=ctx.force_mode or "general",
            metadata={
                "orchestrator_version": "v4+unified",
                "force_mode": force_mode,
                "adaptive_profile": adaptive_profile,
                ...
                "plan": {
                    "subtasks": [
                        {"agent_type": s.agent_type, "query": s.query[:200], "params": s.params}
                        for s in plan.subtasks
                    ],
                    "merge_strategy": plan.merge_strategy,
                },
                "execution_graph": {
                    ...
                    "edges": [
                        {"from": dep, "to": s.sub_question_id}
                        for s in plan.subtasks
                        for dep in (s.depends_on or [])
                    ],
                },
                "fusion": {
                    "method": "fusion_v2" if use_v2_fusion else "fusion_v1",
                    "confidence": final_confidence,
                },
                "critic": {
                    "need_fix": not passed_validation,
                    "feedback": getattr(critic_result if use_v2_fusion else critic_output, "notes", ""),
                    "confidence_breakdown": getattr(critic_result if use_v2_fusion else critic_output, "to_dict", lambda: {})(),
                },
                "metrics": {
                    "first_token_ms": elapsed_ms,
                    "orchestrator_latency_ms": elapsed_ms,
                    ...
                    "avg_agent_latency_ms": int(
                        sum(
                            int((r.metadata or {}).get("dag", {}).get("duration_ms", 0) or 0)
                            for r in agent_results
                        )
    ...
    async def process(self, req: OrchestratorV4Request, event_cb=None) -> OrchestratorV4Response:
        ...
        # ── Phase 2: UnifiedOrchestrator fast path ────────────────────
        if bool(getattr(settings, "kernel_orchestrator_unified_enabled", False)):
            try:
                return await self._process_unified(
                    req, masked_query, adaptive_profile, data_source_context,
                    force_mode, pii_mapping, t0, trace_id, xai_tracer, xai_trace_id,
                    event_cb,
                )
            except Exception as exc:
                logger.error(
                    "UnifiedOrchestrator pipeline failed, falling back to legacy V4: %s",
                    str(exc),
                )
                # Fall through to legacy V4 pipeline below
        ...
        raw_query_for_anchor = str(req.metadata.get("raw_user_query") or req.query)
        relevance_threshold = float(req.metadata.get("relevance_threshold") or 0.35)
        source_set = {r.source for r in tool_results}
        if (
            not force_mode
            and source_set
            and source_set.issubset({"document", "llmwiki", "memory"})
            and not passes_relevance_anchor(
                raw_query_for_anchor,
                fusion.merged_context or "",
                relevance_threshold,
            )
        ):
            fusion.merged_context = ""
            fusion.confidence = 0.0
