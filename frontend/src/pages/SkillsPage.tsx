        ...
        {view === 'upload' && role === 'admin' && (
          <main className="mx-auto max-w-3xl space-y-6 px-4 py-10 sm:px-6">
            ...
            <section className="space-y-4 rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-6">
              <div className="rounded-xl border border-sky-500/20 bg-sky-500/5 p-4 text-xs leading-5 text-sky-700"><p className="font-medium">Skill 包要求</p><p className="mt-1">必须包含一个 SKILL.md；选择文件夹时以目录名作为 Skill 名称。YAML frontmatter 可选；没有时系统只从标题后的首段提取展示摘要，不会改写或蒸馏正文。可同时包含 Markdown、SQL、字段字典和源码等 UTF-8 纯文本参考文件。</p></div>
