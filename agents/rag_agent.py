...
class RagAgent(BaseAgent):
    ...
    async def execute(self, task: TaskMessage) -> AgentResult:
        try:
            ...
            # 🔥 定义类查询降低阈值，避免语义鸿沟导致召回失败
            if self._is_definition_query(query):
                min_score = max(0.20, min_score - 0.10)  # 至少 0.20，避免过低噪声
            ...
            if "documents" in sources:
                ...
                if query_terms:
                    ...
                    search_queries.extend([f"{query} {term}".strip() for term in query_terms[:4]])
                    if any(term in query for term in ["队长", "身份", "权限", "角色"]):
            ...
            memory_intent = any(k in query.lower() for k in ["记忆", "偏好", "之前", "上次", "历史", "用户设置", "profile", "preference"])
            if (doc_evidence_count == 0 and not llmwiki_entries or memory_intent) and ("semantic_memory" in sources or "episodic_memory" in sources):
                ...
                import re
                q_tokens = re.findall(r"[a-z0-9]+|[\u4e00-\u9fff]{1,}", query.lower())
                for m in rows:
                    ...
                    if not hit:
                        if not (mt == "semantic" and getattr(m, "pinned", False) and memory_intent):
                            continue
            ...
            for i, chunk in enumerate(sorted_llmwiki_entries, start=1):
                content_parts.append(
                    f"[LLMWiki {i}] {chunk.get('question', '摘要')} score={float(chunk.get('score', 0.0)):.3f}\n"
                    f"{chunk.get('answer', '')[:200]}"
                )
            for i, chunk in enumerate(sorted_vector_chunks[: max(1, top_k - len(sorted_llmwiki_entries))], start=1):
                content_parts.append(
                    f"[Vector {i}] source={chunk.get('source_type')} id={chunk.get('id')} score={float(chunk.get('score', 0.0)):.3f}\n"
                    f"{chunk.get('text', '')[:200]}"
                )
            if not content_parts:
                for i, chunk in enumerate(sorted_chunks, start=1):
                    content_parts.append(
                        f"[{i}] source={chunk.get('source_type')} id={chunk.get('id')} score={float(chunk.get('score', 0.0)):.3f}\n"
                        f"{chunk.get('text', '')[:200]}"
                    )
            ...
            if not sorted_chunks and query_terms and "documents" in sources:
                ...
                for search_query in [query, " ".join(query_terms[:6])]:
                    doc_chunks = await DocumentPlugin().search_chunks(
                        query=search_query,
                        user_id=user_id,
                        top_k=max(top_k, 8),
                    )
            ...
            confidence = min(0.95, 0.42 + 0.08 * len(sorted_chunks)) if sorted_chunks else 0.35
            avg_score = sum(float(chunk.get("score", 0.0) or 0.0) for chunk in sorted_chunks) / len(sorted_chunks) if sorted_chunks else 0.0
            max_score = max((float(chunk.get("score", 0.0) or 0.0) for chunk in sorted_chunks), default=0.0)
