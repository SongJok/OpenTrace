...
FLAG_REQUIRES: dict[str, tuple[str, ...]] = {
    "kernel_capability_intelligence_phase2_enabled": ("kernel_capability_intelligence_enabled",),
    "kernel_capability_knowledge_graph_enabled": ("kernel_capability_intelligence_phase2_enabled",),
    "kernel_capability_reasoner_enabled": ("kernel_capability_intelligence_phase2_enabled",),
    "kernel_capability_execution_memory_enabled": (
        "kernel_capability_intelligence_phase2_enabled",
    ),
    "kernel_capability_strategy_memory_enabled": ("kernel_capability_intelligence_phase2_enabled",),
    "kernel_capability_evolution_enabled": ("kernel_capability_intelligence_phase2_enabled",),
    "kernel_runtime_cognitive_planner_enabled": ("kernel_runtime_understanding_enabled",),
    "kernel_runtime_evidence_fusion_critic_enabled": ("kernel_runtime_capability_graph_enabled",),
    "kernel_cognitive_planner_v2_enabled": ("kernel_runtime_cognitive_planner_enabled",),
    "kernel_memory_graph_redis_enabled": ("kernel_memory_fabric_retrieval_enabled",),
    "kernel_memory_fabric_primary_only": ("kernel_memory_fabric_retrieval_enabled",),
    "kernel_data_intelligence_routing_enabled": ("kernel_agent_data_enabled",),
    "kernel_agent_capability_executor_mode": ("kernel_runtime_capability_graph_enabled",),
}
...
def validate_feature_flags(settings: Any) -> FlagValidationResult:
    ...
    if _get("kernel_registry_dispatch_strict") and not _get(
...
def export_effective_runtime_flags(settings: Any) -> dict[str, bool]:
    ...
    keys = sorted(k for k in dir(settings) if k.startswith("kernel_") and k.endswith("_enabled"))
