...
class Stage6SyncAnnotationsContractTests(unittest.TestCase):
    def test_chat_response_has_execution_graph_and_annotations(self):
        ...
        self.assertIn("execution_graph: Optional[dict[str, Any]] = None", txt)
        self.assertIn("annotations=(result.metadata.get(\"annotations\", [])", txt)
        self.assertIn("execution_graph=(result.metadata.get(\"execution_graph\")", txt)
