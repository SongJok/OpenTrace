...
def test_legacy_text_embeddings_are_normalized_before_vector_indexing():
    search_indexes = (VERSIONS / "20260714_knowledge_search_indexes.py").read_text(encoding="utf-8")
    normalization = (VERSIONS / "20260725_normalize_vector_columns.py").read_text(encoding="utf-8")
...
def test_user_memory_score_is_added_after_existing_runtime_migrations():
    migration = (VERSIONS / "20260726_add_user_memory_score.py").read_text(encoding="utf-8")
