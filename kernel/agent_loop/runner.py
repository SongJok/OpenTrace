...
class AgentLoop:
    ...
    async def run(
        ...
        *,
        ...
        pending_sql_draft = await self._pending_sql_draft(db, response=response)
        if pending_sql_draft is None:
            available_specs = [spec for spec in available_specs if spec.name != "execute_sql_draft"]
        ...
        sql_draft_request = self._resolve_sql_draft_execution_request(
            query,
            pending_sql_draft,
        )
        client_tool_names = {
        ...
        if sql_draft_request and sql_draft_request.get("status") in {"ready", "clarify"}:
            pinned_names.add("execute_sql_draft")
        ...
        if sql_draft_request and sql_draft_request.get("status") == "ready":
            messages.append(
                LLMMessage(
                    role="system",
                    content=(
                        "用户已经明确选择当前分支中的受治理 SQL 草案候选。只能调用 "
                        "execute_sql_draft，且参数必须使用下方平台绑定值；不得改写 draft_id、"
                        "candidate_ids 或扩大为其它候选。工具会先进入持久审批，恢复后返回的 "
                        "execution_summary.answer 和 answer_citations 是经过真实只读执行与结果验证"
                        "的数据答案，最终回答必须保留其中的 [R1]/[E1] 等引用，不得另造数字。\n"
                        + json.dumps(sql_draft_request["arguments"], ensure_ascii=False)
                    ),
                )
            )
        ...
        if verified_data_answer and set(intent.capabilities).issubset({"execute_sql_draft"}):
            ...
            return AgentLoopResult(
                status="completed",
                content=verified_data_answer,
                model="opentrace-data-answer-projection",
                intent=intent,
                metadata={
                    "model_calls": model_calls,
                    "model_call_count": len(model_calls),
        ...
        public_tools = [
            ...
            and not (data_answer_projection and spec.name == "execute_sql_draft")
        ]
        ...
        if (
            ...
            and (
                self._is_explicit_write_request(query, pending_action=pending_action)
                or bool(sql_draft_request and sql_draft_request.get("status") == "ready")
            )
            and str(payload.get("tool_choice") or "auto") != "none"
        ):
        ...
        for round_number in range(1, round_limit + 1):
            ...
            if sql_draft_request and sql_draft_request.get("status") == "ready":
                calls = [
                    self._bind_sql_draft_execution_call(call, sql_draft_request) for call in calls
                ]
            assistant_calls = [
                {
                    "id": _call_id(call),
