        ...
        async def run_node(node: DagNode) -> AgentResult:
            async with sem:
                ...
                if event_cb is not None:
                    await event_cb({"type": "dag_node_start", "data": {"node_id": node.node_id, "agent_type": node.agent_type, "depends_on": node.depends_on, "started_at_ms": int(started_at * 1000)}})
                ...
                result.metadata = {**(result.metadata or {}), "dag": {"duration_ms": duration_ms, "started_at_ms": int(started_at * 1000), "node_id": node.node_id}}
                return result
        ...
        while pending or inflight:
            ...
            for task in done:
                ...
                try:
                    ...
                    result = AgentResult(task_id=node_id, agent_type="unknown", status="error", content="", error=str(exc), metadata={"dag": {"duration_ms": 0, "node_id": node_id}})
                completed[node_id] = result
                results.append(result)
                if event_cb is not None:
                    duration_ms = int(((result.metadata or {}).get("dag", {}) or {}).get("duration_ms", 0) or 0)
                    await event_cb({"type": "dag_node_complete", "data": {"node_id": node_id, "agent_type": result.agent_type, "status": result.status, "duration_ms": duration_ms, "preview": str(result.content or "")[:200]}})

        for node_id, node in pending.items():
            results.append(AgentResult(task_id=node_id, agent_type=node.agent_type, status="skipped", content="", error="unresolved_dependencies"))
