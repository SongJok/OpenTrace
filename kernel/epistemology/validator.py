...
class OutputValidator:
    def validate_response(self, response: AnnotatedResponse) -> tuple[bool, list[str], AnnotatedResponse]:
        issues: list[str] = []
        for frag in response.fragments:
            text = frag.text or ""
            if re.search(r"```json\s*\{", text, re.IGNORECASE):
                issues.append("[ERROR] 检测到未处理的 JSON 块")
            if frag.annotation and frag.annotation.level == EvidenceLevel.FACT and not frag.annotation.citations:
