...
class SQLValidator:
    ...
    def validate_semantic_correctness(
        self, sql: str, parse_result: SemanticParseResult,
        ...
        for m in parse_result.metrics:
            if m.mapped_column and m.mapped_column.lower() not in lowered:
                issues.append(f"Metric '{m.mapped_column}' (mentioned as '{m.mention}') not found in SQL")
        ...
        if parse_result.time_window and parse_result.time_window.get("days"):
            ...
            if not has_time_sql:
                ...
                issues.append(f"Time filter should use column '{col_hint}' but SQL may use different column")
    ...
    def validate_intent_coverage(self, sql: str, query: str, parse_result: SemanticParseResult) -> dict[str, Any]:
        ...
        report["has_aggregation"] = any(kw in lowered for kw in ("sum(", "count(", "avg(", "max(", "min("))
