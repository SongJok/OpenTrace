...
class TableRelationshipGraph:
    ...
    def _find_heuristic_join(self, table_a: str, table_b: str) -> JoinStep | None:
        ...
        for col_a in cols_a:
            ...
            for col_b in cols_b:
                if not self._is_join_candidate(col_b):
                    continue
                if col_a.lower() in ("created_at", "updated_at") and col_b.lower() in (
                    "created_at",
                    "updated_at",
                ):
                    continue
        ...
        if best_match:
            return JoinStep(
                left_table=table_a,
                right_table=table_b,
                left_key=best_match[0],
                right_key=best_match[1],
            )
        return None
            ...
            if isinstance(raw_tables, list):
                for item in raw_tables:
                    ...
                    for fk in (
                        ...
                        if isinstance(item.get("foreign_keys", []), list)
                        else []
                    ):
                        if isinstance(fk, dict):
                            src = str(fk.get("table") or name).strip()
                            dst = str(
                                fk.get("ref_table") or fk.get("references_table") or ""
                            ).strip()
                            if src:
                                tables.append(src)
                            if dst:
    ...
    def find_join_path(
        self, table_a: str, table_b: str, schema: dict | None = None, schema_hint: str = ""
    ) -> list[JoinStep] | None:
        if table_a == table_b:
